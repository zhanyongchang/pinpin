$().ready( function() {
	...
	box.ClientWidth = 850;	//显示窗口区域宽度。
	box.Step = 1;			//滚动步长。
	box.Speed = 50;			//滚动每步长执行时间（毫秒）。
	box.Space = 0;			//如果使用margin做图片间隔，在此设置间隔距离。
	box.Forward = "left";	//滚动初始方向。向左滚动，值为“left”，向右滚动，值为“right”。
	box.NoRecord = "<div style='line-height:180px;font-size:20px;font-weight:bold;text-align:center;color:#AAA;letter-spacing:20px;'>暂无记录！</div>";		//暂无记录时插入页面的html代码。
	box.Control = true;	//是否可以控制滚动方向。是为true，否为false。
	box.CtrlClass = "ScrollButton";		//定义方向控制按钮的属性class。左右两个按钮设置为相同的class，并且页面中不能再次出现这个class。
	box.CtrlSpeed = 20;		//定义方向控制按钮按下时的速度加倍倍数。
...
$().ready( function() {
	var box = new ScrollBox1("ScrollBox1");	//参数为滚动容器的id，容器必须具备overflow:hidden属性，容器下必须具备标签格式为<ul><li></li>...<li></li></ul>。
	box.ClientWidth = 850;	//显示窗口区域宽度。
	box.Step = 1;			//滚动步长。
	box.Speed = 50;			//滚动每步长执行时间（毫秒）。
	box.Space = 0;			//如果使用margin做图片间隔，在此设置间隔距离。
	...
	box.NoRecord = "<div style='line-height:180px;font-size:20px;font-weight:bold;text-align:center;color:#AAA;letter-spacing:20px;'>暂无记录！</div>";		//暂无记录时插入页面的html代码。
	box.Control = true;	//是否可以控制滚动方向。是为true，否为false。
	box.CtrlClass = "ScrollButton1";		//定义方向控制按钮的属性class。左右两个按钮设置为相同的class，并且页面中不能再次出现这个class。
	box.CtrlSpeed = 20;		//定义方向控制按钮按下时的速度加倍倍数。
	box.Stop = true;		//鼠标悬停时，是否停止。是为true，否为false。
	box.Default();			//初始化。初始化完成后将自动开始滚动。
} );

function ScrollBox1(Box) {
	this.Box = $("#" + Box);
	this.Name = "";
	this.Author = "Keboy";
	this.Version = "1.0";
