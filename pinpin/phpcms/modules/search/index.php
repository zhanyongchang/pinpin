...
class index {
	...
	public function init() {
		...
		if(isset($_GET['q'])) {
			...
			$SEO = seo($siteid, '', $search_q);
			
			if(isset($_GET['mobile'])) {
				if(trim($_GET['mobile'])=='1') {
					$this->wap_site = getcache('wap_site','wap');
					$this->wap = $this->wap_site[$siteid];
					define('WAP_SITEURL', $this->wap['domain'] ? $this->wap['domain'].'index.php?' : APP_PATH.'index.php?m=wap&siteid='.$siteid);
					$WAP = $this->wap;
					include	template('search','waplist');
				}
				else {
