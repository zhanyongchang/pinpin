...
class index {
		...
		*************************/
		...
		if($this->catid) {
		    foreach($this->types as $key => $value) {
			    if($value['cat'] == $this->catid) {
				    $this->typeid = $key;
					break;
				}
			}
			if(!$this->typeid) {
			    $this->typeid = $this->catid;
			}
		}
		else {
		    $this->typeid = intval($_GET['typeid']);
		}
	...
	public function init() {
		...
		$SEO = wapseo($this->siteid,'',$WAP['sitename']);
	...
	public function lists() {
		...
		*************************/
		...
		$template = ($TYPE[$typeid]['parentid']==0 && in_array($typeid,array_unique($parentids))) ? $setting['category_template'] : $setting['list_template'];	
		...
		define('URLRULE', '/mobile/list-{$typeid}.html~/mobile/list-{$typeid}-{$page}.html');
		$GLOBALS['URL_ARRAY'] = array('typeid'=>$typeid);
		
		$pages = wpa_pages($total, $page, $pagesize);
        $SEO = wapseo($this->siteid,$catid);
		include template('wap', $template);
	}	
	...
	public function show() {
		...
		if($pictureurls) {
			...
			if($PIC_POS !== false) {
				...
				//当不存在 [/page]时，则使用下面分页
				$pages = content_pages_wap($pagenumber,$page, $pageurls, 0);
				//判断[page]出现的位置是否在第一位 
				if($CONTENT_POS<7) {
		...
		if($CONTENT_POS !== false) {
			...
			//当不存在 [/page]时，则使用下面分页
			$pages = content_pages_wap($pagenumber,$page, $pageurls);
			//判断[page]出现的位置是否在第一位 
			if($CONTENT_POS<7) {
		...
		if(empty($previous_page)) {
			$previous_page = array('title'=>'没有了', 'thumb'=>IMG_PATH.'nopic_small.gif', 'url'=>'javascript:;');
		}
		if(empty($next_page)) {
			$next_page = array('title'=>'没有了', 'thumb'=>IMG_PATH.'nopic_small.gif', 'url'=>'javascript:;');
		}
		$SEO = wapseo($this->siteid,$catid,$title);
	...
	function page() {  
        ...
        if(!$data) showmessage(L('information_does_not_exist'),'blank');  
        extract($data);
		$CATEGORYS = getcache('category_content_'.$this->siteid,'commons');
		if(!isset($CATEGORYS[$catid])) exit(L('information_does_not_exist','','content'));
