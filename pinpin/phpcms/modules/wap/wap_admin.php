...
class wap_admin extends admin {
	...
	function init() {
		...
		$show_dialog = true;
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=wap&c=wap_admin&a=add\', title:\''.L('add_site').'\', width:\'700\', height:\'550\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('wap_add_site'));		
	...
	function edit() {
		if($_POST['dosubmit']) {
			...
			$wap_bannerwidth = intval($_POST['wap_bannerwidth']) ? intval($_POST['wap_bannerwidth']) : 0;
			$wap_bannerheight = intval($_POST['wap_bannerheight']) ? intval($_POST['wap_bannerheight']) : 0;
			$wap_qrcode = trim($_POST['wap_qrcode']);
			$wap_mapcode = trim($_POST['wap_mapcode']);
			$wap_topcode = trim($_POST['wap_topcode']);
			$wap_bottomcode = trim($_POST['wap_bottomcode']);
			$setting = array2string($_POST['setting']);
			$this->db->update(array('sitename'=>$sitename,'logo'=>$logo,'domain'=>$domain,'wap_bannerwidth'=>$wap_bannerwidth,'wap_bannerheight'=>$wap_bannerheight,'wap_qrcode'=>$wap_qrcode,'wap_mapcode'=>$wap_mapcode,'wap_topcode'=>$wap_topcode,'wap_bottomcode'=>$wap_bottomcode,'setting'=>$setting), array('siteid'=>$siteid));
			$this->wap_site_cache();
	...
	function add() {
		if($_POST['dosubmit']) {
			...
			$wap_bottomcode = trim($_POST['wap_bottomcode']);
			$setting = array2string($_POST['setting']);
			$return_id = $this->db->insert(array('siteid'=>$siteid,'sitename'=>$sitename,'logo'=>$logo,'domain'=>$domain,'wap_bannerwidth'=>$wap_bannerwidth,'wap_bannerheight'=>$wap_bannerheight,'wap_qrcode'=>$wap_qrcode,'wap_mapcode'=>$wap_mapcode,'wap_topcode'=>$wap_topcode,'wap_bottomcode'=>$wap_bottomcode,'setting'=>$setting),'1');
			$this->wap_site_cache();
	...
	function type_manage() {
		...
		if($_POST['dosubmit']) {
			...
			foreach($result as $r) {
				...
				$r['add_new'] = '<a href="#" onclick="add_tr(this,\''.$r['typeid'].'\',\''.$r['siteid'].'\');">'.L('wap_add_subtype').'</a>';
			...
			$str  = "<tr>
						...
						<td align='left'>\$spacer<input name='typename[\$typeid]' type='text' value='\$typename' class='input-text' size='20' >\$add_new</td>
