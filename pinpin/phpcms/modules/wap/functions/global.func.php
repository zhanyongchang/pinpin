 ...
 * 获取类名
 */
function wap_typename($typeid = NULL, $siteid = '') {
 ...
 * 获取子分类根据typeid
 */
function subtypeid($typeid = NULL, $siteid = '') {
	if (empty($siteid)) $siteid = $GLOBALS['siteid'];
	$types = getcache('wap_type','wap');
	if($types[$typeid]['parentid']!=0) {
      $typeid=$types[$typeid]['parentid'];
      foreach($types as $id=>$type) {
		if($type['siteid'] == $siteid && ($typeid === NULL || $type['parentid'] == $typeid)) {
			$subtype[$id] = $type;
		}		
	  }
	}
...
function wpa_pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
	    ...
	    $multipage .= '共<strong> 1 </strong>页 <strong>'.$num.'</strong>条记录';
	}
	return $multipage;
}
 ...
 * 生成文章分页方法
 */
function content_pages_wap($num, $curr_page,$pageurls,$showremain = 1) {
	$multipage = '';
	$page = 11;
	$offset = 4;
	$pages = $num;
 ...
 * @param $keyword      关键词
 */
function wapseo($siteid, $catid = '', $title = '', $description = '', $keyword = '') {
