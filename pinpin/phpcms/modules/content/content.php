...
class content extends admin {
	...
	public function __construct() {
		...
		$this->db_hits = pc_base::load_model('hits_model');
	...
	public function edit() {
			...
			if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
				...
				//将新文章的数据插入hits表进行点击数统计(zhan)
				$hitsid = "c-".$catid."-".$id;
				$r = $this->db_hits->count("hitsid='$hitsid'");
				if($r==0){
					$updatetime = time();
					$this->db->query("INSERT INTO jzk_hits (hitsid, catid, updatetime) VALUES ('$hitsid', '$catid', '$updatetime')");
				}else{}
