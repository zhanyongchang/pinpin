...
class index {
	...
	public function init() {
		...
		//判断域名是否填写正确----------begin
		if(isset($_GET['upcache']) && $siteid == 1) {
			$this->sitedb = pc_base::load_model('site_model');
			$currentsite = $this->sitedb->get_one(array('siteid'=>$siteid));
			if(empty($currentsite)) {
				$currentsite=$siteinfo;
			}
			$domain = strtolower(rtrim(trim($currentsite['domain']),'/'));             //获取数据库中domain字段的值
			$path_info = strtolower(rtrim(trim("http://".$_SERVER['HTTP_HOST']),'/')); //获取地址栏域名
			if($domain != $path_info) {
				$path_info = $path_info."/";
				$sql = array('domain'=>$path_info);
				$this->sitedb->update($sql, array('siteid'=>$siteid));
				...
				$modules = array(
					...
					array('name' => L('wapsites'), 'function' => 'cache_wapsite'),
				);
				...
				for ($page=0; $page<$length; $page++) {
					...
					if ($m['mod'] && $m['function']) {
						...
						$cache_api->cache($m['function'], $m['param']);
					}
				}
				//生成首页html
				$this->html = pc_base::load_app_class('html');
				$size = $this->html->index();
				$size += $this->html->indexwap();
				$size += $this->html->indexwapindex();
			}
		}
		//判断域名是否填写正确----------end
	...
	public function lists() {
		...
		if($type==0) {
			...
			$SEO = seo($siteid, $catid, 0, $setting['meta_description'],$keywords);
			include template('content',$template);
		}
	}
