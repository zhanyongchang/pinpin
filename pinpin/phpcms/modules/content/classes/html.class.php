...
class html {
	 ...
	 * 更新wap首页
	 */
	public function indexwap() {
		if($this->wap['status']==1 && $this->siteid==1)  { 
			$WAP = $this->wap;
			$TYPE = $this->types;
			$file = PHPCMS_PATH.'mobile/index.html';
			//添加到发布点队列
			$this->queue->add_queue('edit','/mobile/index.html',$this->siteid);
			$siteinfo = siteinfo($this->siteid);
			//SEO
	 ...
	 * 更新wap首页
	 */
	public function indexwapindex() {
		if($this->wap['status']==1 && $this->siteid==1)  { 
			$WAP = $this->wap;
			$TYPE = $this->types;
			$file = PHPCMS_PATH.'mindex.html';
			//添加到发布点队列
			$this->queue->add_queue('edit','mindex.html',$this->siteid);
			$siteinfo = siteinfo($this->siteid);
			//SEO
			$SEO = wapseo($this->siteid,'',$WAP['sitename']);
			$siteid = $this->siteid;
