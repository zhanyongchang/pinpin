...
function challs_flash_onComplete(a){ //每次上传完成调用的函数，并传入一个Object类型变量，包括刚上传文件的大小，名称，上传所用时间,文件类型
	...
	var size=(a.fileSize/1024).toFixed(2); //获取上传文件大小，单位KB
	...
	document.getElementById('show').innerHTML+=name+' --- '+size+'KB ----文件类型：'+type+'--- 用时：'+(time/1000)+'秒<br><br>'
}
...
function challs_flash_onCompleteAll(a){ //上传文件列表全部上传完毕事件,参数 a 数值类型，返回上传失败的数量
	...
	var typeid=myselect.options[index].value;
	if(typeid=="0") {
	    document.getElementById('show').innerHTML+='<font color="#ff0000">请选择档案的主类别！</font><br />';
	}
	else {
	    document.getElementById('show').innerHTML+='<font color="#ff0000">所有文件上传完毕，</font>上传失败'+a+'个！<br />';
	}
...
function thisMovie(movieName){   
  if(isMSIE){   
    ...
    <div align="center">
        <p><a href="index.php?m=content&c=tproA&pc_hash=<?php echo $_SESSION['pc_hash']?>">列表模式</a></p>	
<?php echo form::select_category('category_content_'.$this->siteid,$parentid,'name="info[parentid]" id="select"',"请选择档案的主类别",0,0);
?>
        <br />
        <br />
        <object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=11,0,0,0" width="442" height="450" id="update" align="middle">
            <param name="allowFullScreen" value="false" />
            <param name="allowScriptAccess" value="always" />
            <param name="movie" value="api/update.swf" />
            <param name="quality" value="high" />
            <param name="bgcolor" value="#ffffff" />
            <embed src="api/update.swf" quality="high" bgcolor="#ffffff" width="442" height="450" name="update" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
        </object>
