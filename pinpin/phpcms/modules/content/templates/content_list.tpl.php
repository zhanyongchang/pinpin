...
<div class="table-list">
    <table width="100%">
        <thead>
            <tr>
			<th width="6%"><input type="checkbox" value="" id="check_box" onclick="selectall('ids[]');"></th>
            <th width="6%"><?php echo L('listorder');?></th>
			<th><?php echo L('title');?></th>
            <th width="8%"><?php echo L('hits');?></th>
            <th width="13%"><?php echo L('publish_user');?></th>
            <th width="18%"><?php echo L('updatetime');?></th>
			<th width="12%"><?php echo L('operations_manage');?></th>
            </tr>
        </thead>
<tbody>
    <?php
	...
	?>
        <tr onMouseMove="javascript:getId('<?php echo $r['id']?>').style.display='block';" onMouseOut="javascript:getId('<?php echo $r['id']?>').style.display='none';">
		<td align="center"><input class="inputcheckbox " name="ids[]" value="<?php echo $r['id'];?>" type="checkbox"><?php echo $r['id'];?></td>
        <td align='center'><input name='listorders[<?php echo $r['id'];?>]' type='text' size='3' value='<?php echo $r['listorder'];?>' class='input-text-c'></td>
		<td><input name="title" type="text" value="<?php echo $r['title'];?>" data="<?php echo $r['id']?>" data-catid="<?php echo $catid;?>" data-siteid="<?php echo $this->siteid; ?>" style="display:none;width:80%;" maxlength="255" /><span><?php echo $r['title']?></span><img src="/statics/images/admin_img/tdedit.gif" class="img1" style="cursor:pointer;" /><?php if($r['thumb']!='') {echo '<img src="'.IMG_PATH.'icon/small_img.gif" title="'.L('thumb').'">'; } if($r['posids']) {echo '<img src="'.IMG_PATH.'icon/small_elite.gif" title="'.L('elite').'">';} if($r['islink']) {echo ' <img src="'.IMG_PATH.'icon/link.png" title="'.L('islink_url').'">';}?></td>
		...
		<td align='center'><a href="javascript:;" onclick="javascript:openwinx('?m=content&c=content&a=edit&catid=<?php echo $catid;?>&id=<?php echo $r['id']?>','')"><?php echo L('edit');?></a> | <a href="javascript:view_comment('<?php echo id_encode('content_'.$catid,$r['id'],$this->siteid);?>','<?php echo safe_replace($r['title']);?>')"><?php echo L('comment');?></a>
         <div id='<?php echo $r['id']?>' style="position:absolute;left:50%;height:100px;z-index:99;display:none;">
           <img src='<?php echo $r['thumb']?>' onload="DrawImage(this,300,225)" border="0" alt="" />
         </div></td>
	</tr>
     <?php }
	}
	...
	$("td img.img1").click(function () {
		...
		if (img) {
			...
			text.blur(function () {
				...
				var catid = text.attr("data-catid");
				var siteid = text.attr("data-siteid");
		        if($.trim(text.val()) != $.trim(span.text())) {
					$.ajax({
						type: "GET",
						contentType: "application/json",
						url: "/api.php",
						data:"op=edit_title&Title="+encodeURIComponent(title)+"&Id="+id+"&CatId="+catid+"&SiteId="+siteid,
						dataType: 'json',
						success: function(re) {
