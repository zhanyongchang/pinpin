...
class index {
	...
	public function post() {
		...
		$username = param::get_cookie('username',$SITE['name'].L('phpcms_friends'));
		...
		//每次进行评论后将总评论数更新到news表中(zhan)
		$db_comment = pc_base::load_model('comment_model');
		$r = $db_comment->get_one("url='$url'");
		$r_total = $r['total'];
		$db_comment->query("update jzk_news set comment_num='$r_total' where url='$url'");
