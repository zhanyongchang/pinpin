...
class poster extends admin {
	...
	public function init() {
		...
		$infos = $this->db->listinfo($where, '`listorder` ASC, `id` DESC', $page);
	 ...
	 */
	public function add() {
		$spaces = $this->s_db->select('`disabled`=0');
		if (isset($_POST['dosubmit'])) {
			$poster = $this->check($_POST['poster']);
			if (!isset($poster['spaceid']) || empty($poster['spaceid'])) showmessage('请选择所属版位', 'index.php?m=poster&c=poster&a=add');
			...
			if ($id) {
				...
				showmessage(L('add_ads_success'), 'index.php?m=poster&c=poster&a=init');
			} else {
				showmessage(L('operation_failure'), 'index.php?m=poster&c=poster&a=init');
			}
		} else {
			$spaceid = $_GET['spaceid'] ? intval($_GET['spaceid']) : 1;
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			showmessage(L('edit_ads_success'), 'index.php?m=poster&c=poster&a=init');
		} else {
			...
			case 'images':
				...
				if(is_array($setting['images'])) {
					...
					foreach ($setting['images'] as $k => $s) {
						if($s['linkurl']=='http://' || $s['linkurl']=='') {
							$setting['images'][$k]['linkurl'] = '/';
						}
						if (!$s['imageurl']) {
							$setting['images'][$k]['imageurl'] = '';
							$tag = 1;
						}
						else {
						    $tag = 1;
