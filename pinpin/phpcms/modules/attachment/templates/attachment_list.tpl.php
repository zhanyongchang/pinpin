...
<form name="myform" action="?m=admin&c=role&a=listorder" method="post" id="myform">
    <table width="100%" cellspacing="0">
        <thead>
		<tr>
		<th width="5%"><?php echo L('delete')?></th>
		<th width="5%">ID</th>
...
?>
<tr onMouseMove="javascript:getId('<?php echo $info['aid']?>').style.display='block';" onMouseOut="javascript:getId('<?php echo $info['aid']?>').style.display='none';">
<td width="5%" align="center"><input type="checkbox" name="aid[]" value="<?php echo $info['aid']?>" id="att_<?php echo $info['aid']?>" /></td>
...
<td  align="center"><a href="javascript:preview(<?php echo $info['aid']?>, '<?php echo $info['filename']?>','<?php echo $this->upload_url.$info['filepath']?>')"><?php echo L('preview')?></a> | <a href="javascript:;" onclick="att_delete(this,'<?php echo $info['aid']?>')"><?php echo L('delete')?></a>
    <div id='<?php echo $info['aid']?>' style="position:absolute; left:280px;height:100px;z-index:99; display:none;">
           <img src='<?php echo $this->upload_url.$info['filepath']?>' onload="DrawImage(this,300,225)" border="0" alt="" />
         </div></td>
</tr>
<?php 
	}
