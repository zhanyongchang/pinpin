		...
		$CATEGORYS = getcache('category_content_'.$siteid,'commons');
 		if(isset($_POST['dosubmit'])){
			...
			$qq = safe_replace(strip_tags($_POST['qq']));
 			if($_POST['linktype']=='0'){
 				$sql = array('siteid'=>$siteid,'typeid'=>intval($_POST['typeid']),'linktype'=>intval($_POST['linktype']),'name'=>$name,'addtime'=>$addtime,'url'=>$url,'username'=>$username,'email'=>$email,'qq'=>$qq,'ip'=>$ip);
 			}else{
 				$sql = array('siteid'=>$siteid,'typeid'=>intval($_POST['typeid']),'linktype'=>intval($_POST['linktype']),'name'=>$name,'addtime'=>$addtime,'url'=>$url,'username'=>$username,'email'=>$email,'qq'=>$qq,'ip'=>$ip,'logo'=>$logo);
 			}
 			$link_db->insert($sql);
