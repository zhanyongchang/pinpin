...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		$multipage .= '<a class="a1">'.$num.L('page_item').'记录</a>';
		if($curr_page>0) {
 ...
 * 获取图片路径
 * @param  $imgurl 图片路径
 * @param  $smallpic 无图片时默认图片路径
 */
function getImgPath($imgurl, $smallpic = 'nopic.gif') {
