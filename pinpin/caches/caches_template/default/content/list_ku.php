    ...
    <div class="shuaixuan">
        <div class="shuaixuan_top">
            ...
            <div class="item" style="display: block;" id="video_uptime">
                ...
                <ul>
                    <li class="li_active" data-val="0"><a href="javascript:;">全部</a></li>
                    <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=8f65e0109f3c74bd2d0030ae1ff17db2&sql=select+%2A+from+jzk_model_field+where+fieldid%3D119\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("select * from jzk_model_field where fieldid=119 LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
                    <?php $n=1;if(is_array($data)) foreach($data AS $zz) { ?>
                        <?php $yy = string2array($zz[setting]);?>
                        <?php $xx = explode("\r\n",$yy[options]);?>
                        <?php $n=1;if(is_array($xx)) foreach($xx AS $ww) { ?>
        ...
        <div class="shuaixuan_content">
            ...
            <div class="show_content">
                <ul>
                    <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=9e0df984f47a36c85c2fcd1e2da3e422&action=lists&catid=1&order=listorder+desc\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$data = $content_tag->lists(array('catid'=>'1','order'=>'listorder desc','limit'=>'20',));}?>
