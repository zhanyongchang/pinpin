<?php defined('IN_PHPCMS') or exit('No permission resources.'); ?><?php include template("content","header"); ?>
    ...
    <div class="main">
        <div class="indexCenter">
            <h2 class="comment-title blue mt10"><a href="<?php if($comment[url]) { ?><?php echo $comment['url'];?><?php } else { ?><?php echo $url;?><?php } ?>"><?php if($comment[title]) { ?><?php echo $comment['title'];?><?php } else { ?><?php echo $title;?><?php } ?></a> <a href="#comment" class="f12 fn"><font color="#FF0000">[我来说两句]</font></a></h2>
...
<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
                    ...
                    <div class="content"><?php echo stripcslashes($r[content]);?>
                        <div class="rt"><a href="javascript:void(0)" onclick="reply(<?php echo $r['id'];?>, '<?php echo $commentid;?>')">回复</a>  <a href="javascript:void(0)" onclick="support(<?php echo $r['id'];?>, '<?php echo $commentid;?>')">支持</a>（<font id="support_<?php echo $r['id'];?>"><?php echo $r['support'];?></font>）</div>
...
<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
                ...
                <div class="comment-form">
                    ...
                    <input type="hidden" name="url" value="<?php echo urlencode(($comment[url] ? $comment[url] : $url));?>">
                        ...
                        <div class="posn">我的态度：<input type="radio" name="direction" value="1" /> <img src="<?php echo IMG_PATH;?>icon/zheng.png" /> <input type="radio" name="direction" value="2" /> <img src="<?php echo IMG_PATH;?>icon/fan.png" /> <input type="radio" name="direction" value="3" checked /> <img src="<?php echo IMG_PATH;?>icon/zhong.png" /></div>
                        <textarea rows="5" cols="80" name="content" id='txtContent'></textarea><br />
                        <?php if($setting[code]) { ?>
		
                        <label>验证码：<input type="text" name="code"  class="input-text" id="yzmText" onfocus="var offset = $(this).offset();$('#yzm').css({'left': +offset.left-8, 'top': +offset.top-$('#yzm').height()});$('#yzm').show();$('#yzmText').data('hide', 1)" onblur='$("#yzmText").data("hide", 0);setTimeout("hide_code()", 3000)' /></label>
                        <div id="yzm" class="yzm"><?php echo form::checkcode();?><br />点击图片更换</div>
                        <div class="bk10"></div>
                        <?php } ?>
                        
                        <div class="btn mt5"><input type="submit" value="发表评论" /></div>
                    </form>
...
<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
                        <li>·<a href="<?php echo $r['url'];?>" title="<?php echo $r['title'];?>"><?php echo str_cut($r[title], 60, '...');?></a>(<?php echo $r['total'];?>)</li>
<?php $n++;}unset($n); ?>
                    </ul>
<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
                </div>
